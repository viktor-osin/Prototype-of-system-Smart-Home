...
int s1=0, pas=0, sig, value1, ihome, r;
...
byte val; //константа входящих кодов
...
void loop() {
  ...
  if(sigvoice == 1)//если включена сигнализация
  { 
    sgv++;
    if(sgv >= 900) //повтороение звукового сообщения
    {
      sgv = 0;
...
//контроль температуры, времени, занавесок и вентиляции каждые 0,5сек
  clocktime++;
  if(clocktime >= 500)
  {
    clocktime = 0;
...
if(Serial.available() > 0)  //если что-то пришло в сериал
{
  ...
  delay(10);
 }else password(); //если пароль не верный, то функция введения пароля
}
